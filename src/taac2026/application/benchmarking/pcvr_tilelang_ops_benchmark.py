"""Benchmark TileLang-backed PCVR operators on synthetic tensors (Windows — torch backend only)."""
...
BenchmarkBackend = Literal["torch"]
...
DEFAULT_BACKENDS: tuple[BenchmarkBackend, ...] = ("torch",)
SUPPORTED_BACKENDS: tuple[BenchmarkBackend, ...] = ("torch",)
...
def _parse_backend_names(value: str) -> tuple[BenchmarkBackend, ...]:
    ...
    if invalid:
        raise ValueError(f"unsupported backends on Windows: {', '.join(invalid)}. Only 'torch' is supported.")
...
def _prepare_rms_norm_callable(
    *,
    ...
    raise RuntimeError(f"backend '{backend}' is not available on Windows")
