...
class RMSNorm(nn.Module):
    ...
    Alternative to LayerNorm that only scales (no bias), often more stable
    for deep transformer training.
    """
...
class SENET(nn.Module):
    """Squeeze-and-Excitation Network for channel-wise attention.

    Adapts feature importance per sample based on global context.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: (B, T, D) where T = num_tokens, D = d_model.

        Returns:
            Attended tensor of shape (B, T, D).
        """
        B, T, D = x.shape
        # Squeeze: global average pooling over tokens
        squeeze = x.mean(dim=1)  # (B, D)
        # Excitation: learnable gating
        excitation = self.fc(squeeze)  # (B, D)
        # Scale: apply channel-wise attention
        x = x * excitation.unsqueeze(1)  # (B, T, D)
        return x


class FeatureCrossBlock(nn.Module):
    """Explicit 2nd-order Feature Cross layer (FM-style).

    Computes pairwise feature interactions:
        cross = 0.5 * (sum(z)^2 - sum(z^2))
    where z = Linear(tokens).

    This provides explicit feature crossing without the d_model % T constraint.
    """
    ...
    def __init__(
        ...
        # Linear projection for feature crossing
    ...
    def forward(self, Q: torch.Tensor) -> torch.Tensor:
        """Applies explicit feature crossing.
        ...
        # 1. Linear projection
        z = self.cross_proj(Q)  # (B, T, D)

        # 2. Compute 2nd-order interactions: 0.5 * (sum(z)^2 - sum(z^2))
        # sum over tokens dimension
        sum_z = z.sum(dim=1, keepdim=True)  # (B, 1, D)
        sum_z_sq = (z ** 2).sum(dim=1, keepdim=True)  # (B, 1, D)
        cross = 0.5 * (sum_z ** 2 - sum_z_sq)  # (B, 1, D)

        # 3. Broadcast cross to all tokens and add to original
        Q_cross = Q + cross.expand(-1, T, -1)  # (B, T, D)
...
class RankMixerBlock(nn.Module):
    ...
    def __init__(
        ...
        hidden_mult: int = 4,
        dropout: float = 0.0,
        mode: str = 'full'  # 'full' | 'ffn_only' | 'none' | 'fm'
    ) -> None:
        super().__init__()
        self.T = n_total
        self.D = d_model
        ...
        if mode == 'fm':
            # Use FeatureCrossBlock instead
            self.fm_block = FeatureCrossBlock(
                d_model=d_model,
                n_total=n_total,
...
class DropPath(nn.Module):
    ...
    drop_path_rate. During inference, acts as identity.
    """

    def __init__(self, drop_path_rate: float = 0.0) -> None:
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.drop_path_rate == 0.0 or not self.training:
            return x
        # Generate binary mask: keep with probability (1 - drop_path_rate)
...
class MultiSeqHyFormerBlock(nn.Module):
    ...
    def __init__(
        ...
        # SENET channel attention (applied after token fusion)
        self.senet = SENET(num_channels=d_model, reduction_ratio=4)
        self.use_senet = False  # Disabled by default, enable via config

        # Stochastic Depth for sequence encoder and cross-attention paths
        self.drop_path_enc = DropPath(drop_path_rate)
        self.drop_path_cross = DropPath(drop_path_rate)
        self.drop_path_mixer = DropPath(drop_path_rate)
    ...
    def forward(
        ...
        # 1. Independent Sequence Evolution per sequence (with DropPath)
        next_seqs = []
        next_masks = []
        for i in range(S):
            ...
            # Apply DropPath: randomly drop the entire sublayer output
            next_seq_i = seq_tokens_list[i] + self.drop_path_enc(next_seq_i - seq_tokens_list[i])
            next_seqs.append(next_seq_i)
            next_masks.append(mask_i)

        # 2. Independent Query Decoding per sequence (with DropPath)
        decoded_qs = []
        for i in range(S):
            rc = rope_cos_list[i] if rope_cos_list is not None else None
            rs = rope_sin_list[i] if rope_sin_list is not None else None
            ...
            # Apply DropPath: randomly drop the entire sublayer output
            decoded_q_i = q_tokens_list[i] + self.drop_path_cross(decoded_q_i - q_tokens_list[i])
        ...
        # 3.5 SENET channel attention (optional)
        if self.use_senet:
            combined = self.senet(combined)

        # 4. Query Boosting (with DropPath)
        boosted = self.mixer(combined)  # (B, Nq*S + Nns, D)
        # Apply DropPath: randomly drop the entire mixer output
        boosted = combined + self.drop_path_mixer(boosted - combined)
    ...
    def forward_ns_only(self, ns_tokens: torch.Tensor) -> torch.Tensor:
        """当 num_sequences=0 时，只处理 NS tokens 的 RankMixer.
        ...
        Returns:
            boosted_ns: (B, Nns, D)
        """
        combined = ns_tokens  # (B, Nns, D)
        boosted = self.mixer(combined)  # (B, Nns, D)
        return boosted
...
class RankMixerNSTokenizer(nn.Module):
    ...
    def forward(self, int_feats: torch.Tensor) -> torch.Tensor:
        ...
        """
        # Ensure float input for embedding lookup compatibility
        if int_feats.dtype != torch.long:
            int_feats = int_feats.long()
        ...
        for group in self.groups:
            for fid_idx in group:
                ...
                if emb_real_idx == -1:
                    ...
                    emb_layer = self.embs[emb_real_idx]
                    if length == 1:
                        fid_emb = emb_layer(int_feats[:, offset])
                    else:
        ...
        cat_emb = cat_emb.float()  # Ensure float dtype before projection
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Note: 'fm' mode does NOT require d_model % T == 0
        ...
        if self.num_sequences > 0:
            output_proj_input_dim = num_queries * self.num_sequences * d_model
        else:
            output_proj_input_dim = self.num_ns * d_model
        self.output_proj = nn.Sequential(
            nn.Linear(output_proj_input_dim, d_model),
            nn.LayerNorm(d_model),
        )
    ...
    def _run_multi_seq_blocks(
        ...
        # Handle no-sequence case (e.g., Criteo dataset)
        if self.num_sequences == 0:
            curr_ns = ns_tokens
            if apply_dropout:
                curr_ns = self.emb_dropout(curr_ns)
            for block in self.blocks:
                curr_ns = block.forward_ns_only(curr_ns)
            B = curr_ns.shape[0]
            output = curr_ns.view(B, -1)  # (B, num_ns * D)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # 2. Embed each sequence domain (dynamic) - skip if no sequences
        seq_tokens_list = []
        seq_masks_list = []
        for domain in self.seq_domains:
