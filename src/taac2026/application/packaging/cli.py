...
def _write_training_entrypoint(workspace_root: Path, target_path: Path) -> None:
    run_sh = workspace_root / "run.sh"
    run_ps1 = workspace_root / "run.ps1"
    if run_sh.exists():
        shutil.copy2(run_sh, target_path)
    elif run_ps1.exists():
        shutil.copy2(run_ps1, target_path)
    else:
        raise FileNotFoundError(f"Neither run.sh nor run.ps1 found in {workspace_root}")
